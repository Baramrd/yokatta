...
export default function MangaDetailView({ mangaId }: ViewProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      setIsLoading(true);
      try {
        // Gunakan prop 'mangaId' untuk fetch data
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8">
      <aside className="w-full md:w-1/4">
        ...
        />
        <button
          onClick={() => toggleBookmark(numericMangaId)}
          className={`w-full mt-4 p-2 rounded-lg font-bold transition-colors ${
            isBookmarked(numericMangaId)
      ...
      <section className="w-full md:w-3/4">
        ...
        <div className="flex flex-wrap gap-2 my-4">
            {manga.genres.map((genre: Genre) => (
                <Link href={`/?genre=${genre.mal_id}`} key={genre.mal_id} className="px-3 py-1 bg-secondary text-primary rounded-full text-sm hover:bg-accent">
                  {genre.name}
                </Link>
            ))}
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {chapters.length > 0 ? (
            chapters.map((chapterNumber) => (
              <div key={chapterNumber} className="bg-secondary p-4 rounded-lg hover:bg-accent transition-colors cursor-pointer text-center">
